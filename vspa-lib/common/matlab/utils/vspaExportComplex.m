...
function [size] = vspaExportComplex(in, prec, filePath)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [size] = vspaExportComplex(in, prec, filePath)
%
% DESCRIPTION:
% 
% INPUTS:
%   in       - input complex buffer
...
% Convert precision
sizeHW = length(in(:)) * 2 * elem_size;   % Size (in Half Words)
sizeFW = sizeHW / 2;                      % Size (in Full Words)
dmemIn = dmemCreate(0, sizeFW);
dmemIn = dmemWriteComplex(dmemIn, 0, in(:), prec);
