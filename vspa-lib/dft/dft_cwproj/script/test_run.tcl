...
set ReportPath    "../../doc/DFT_Report.csv"
...
for {set TC_ind $TC_NUM_START} {$TC_ind <= $TC_NUM_END} {incr TC_ind} {
    ...
    # ============================ Load input data =============================
    # Load input buffers
    restore -b "${tcDirPathIn}/input.bin" m:[evaluate #x INP_BUFF] 16bit
	...
	set out_size [evaluate #d  TEST_CTRL.n_dft]
	
	set n_dft     [evaluate #d  TEST_CTRL.n_dft]
    set inp_prec  [evaluate #d  TEST_CTRL.inp_prec]
    set out_prec  [evaluate #d  TEST_CTRL.out_prec]


    # Get output size (half words)    
    if {$out_prec == $HALF_FIXED} then {
        set elem_size  1
    } elseif {$out_prec == $HALF_FLOAT} then {
        set elem_size  1
    } elseif {$out_prec == $SINGLE_PRECISION} then {
        set elem_size  2
    } elseif {$out_prec == $DOUBLE_PRECISION} then {
        set elem_size  4
    }        
	
	
	
	set out_size [evaluate #d $n_dft * 2 * $elem_size]
    ...
    if {$diffResult != $DIFF_FILE_MATCH} then {
        ...
        lappend Failures "DFT error for TC ${TC_num} ..."
    }
