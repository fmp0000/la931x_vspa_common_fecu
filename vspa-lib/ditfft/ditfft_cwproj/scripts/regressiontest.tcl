...
set fftsz_set {128 512 1024 2048}
...
foreach fftsz $fftsz_set {
	...
	if {$fftsz == 512} {
		puts "** Running test for $fftsz pt FFT with SFLIN SFLOUT"
	    set output_size_word [expr $fftsz*2]
...
foreach fftsz $fftsz_set {
	...
	puts "** Running test for $fftsz pt IFFT with HFXIN SFLINTERMEDIATE HFXOUT"
    set output_size_word [expr $fftsz]
	...
	if {$fftsz == 512} {
		puts "** Running test for $fftsz pt IFFT with SFLIN SFLOUT"
	    set output_size_word [expr $fftsz*2]
	    set test_name "fft$fftsz\_type2"
		set tempres [source "fft.tcl"]
		set regressres [expr $regressres&$tempres]
...
if {$regressres == 1} {
	puts "DIT-FFT Regression Test PASS"
} else {
	puts "DIT-FFT Regression Test FAIL"
}
