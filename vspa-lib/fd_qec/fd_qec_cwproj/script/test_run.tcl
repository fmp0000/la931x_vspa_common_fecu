...
# ==============================================================================
#! @file            test_run.tcl
#! @brief           TCL script for testing LTF correlation implementation.
# ==============================================================================
...
set ReportPath    "../../doc/fd_qec_Report.csv"
...
jenkinsWriteReport "fd_qec_Report.csv" "Test Case,Total cycles,Test Status"
...
for {set TC_ind $TC_NUM_START} {$TC_ind <= $TC_NUM_END} {incr TC_ind} {
    ...
    restore -b "${tcDirPathIn}/ctrl.bin" m:[evaluate #x FD_QEC_TEST_CTRL] 16bit

    # Load input buffer
    restore -b "${tcDirPathIn}/input.bin" m:[evaluate #x FD_QEC_INP_BUFF] 16bit
    ...
    # Get params
    set inputLen [evaluate #d FD_QEC_TEST_CTRL.inpLen]
    
    radix D
    # Store output

    save -b m:[evaluate #x FD_QEC_OUT_BUFF]#$inputLen "${tcDirPathOut}/output.bin" -o 16bit
    
    # Set local error count
    set LocalErrorCount  0
    
    # Verify correlation power
    ...
    if {$diffResult != $DIFF_FILE_MATCH} then {
        ...
        lappend Failures "FD_QEC output error for TC ${TC_num} ..."
    }
    ...
    jenkinsWriteReport "fd_qec_Report.csv" "$TC_num, $numCycles, $test_status"
}
