...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function [y_out_vsp, y_out_mat, opstruct]  = r_customsigcond(x, bw, decim_taps, ipstruct)
% 
% DESCRIPTION:
...
opstruct = struct('y_stage1', [], 'y_stage2', [], 'y_stage2a', [], 'y_stage3', [], 'y_stage4', []);

%% Stage 1: DC offset and gain stage

% MATLAB implementation
y_stage1_mat = fegain*x - dcoff;

% VSPA implementation
y_stage1_vsp = r_half(r_smad(r_single(fegain), x_vsp, -r_single(dcoff)));
opstruct.y_stage1 = y_stage1_vsp;

if (bw == 160)
    y_lastddcstage_vsp = y_stage1_vsp;
end

%% Stage 2: Decimate and mix
if (bw <= 80)
    % first decimation stage: MATLAB implementation
    [~, temp] = r_decim2x([y_stage1_mat; zeros(32, 1)], h1_dbl);
    y_stage2_mat = temp(1:n_samples_in2);
        
    % first decimation stage: VSPA implementation
    temp = r_decim2x([y_stage1_vsp; zeros(32, 1)], h1_vsp);
    y_stage2_vsp = temp(1:n_samples_in2);
    opstruct.y_stage2 = y_stage2_vsp;

    % mixer stage: MATLAB implementation
    ncoSeq = exp(-sqrt(-1)*2*pi*(normphase + (0:n_samples_in2-1))*normfreq).';
    y_stage2a_mat = r_half(r_smad(y_stage2_mat, ncoSeq, 0));
    
    % mixer stage: VSPA implementation
    ncoSeq = r_nco(normfreq, n_samples_in2, normphase).';
    y_stage2a_vsp = r_half(r_smad(y_stage2_vsp, ncoSeq, 0));
    opstruct.y_stage2a = y_stage2a_vsp;
    
    y_lastddcstage_mat = y_stage2a_mat;
    y_lastddcstage_vsp = y_stage2a_vsp;
end

%% Stage 3: Decimate
if (bw <= 40)
    % MATLAB implementation
    [~, temp] = r_decim2x([zeros(32, 1); y_stage2a_mat(1:n_samples_in2-1)], h2_dbl);
    y_stage3_mat = temp(1:n_samples_in3);
    
    % VSPA implementation
    temp = r_decim2x([zeros(32, 1); y_stage2a_vsp(1:n_samples_in2-1)], h2_vsp);
    y_stage3_vsp = temp(1:n_samples_in3);
    opstruct.y_stage3 = y_stage3_vsp;
    
    y_lastddcstage_mat = y_stage3_mat;
    y_lastddcstage_vsp = y_stage3_vsp;
end

%% Stage 4: Decimate
if (bw == 20)
    % MATLAB implementation
    [~, temp] = r_decim2x([zeros(32, 1); y_stage3_mat(1:n_samples_in3-1)], h3_dbl);
    y_stage4_mat = temp(1:n_samples_in4);
    
    % VSPA implementation
    temp = r_decim2x([zeros(32, 1); y_stage3_vsp(1:n_samples_in3-1)], h3_vsp);
...
%% Stage 5: IQ imbalance compensation
n_ssfilter_taps = length(iqimb_ssdelayfilt);

% MATLAB implementation
temp = [zeros(n_ssfilter_taps-1, 1); y_lastddcstage_mat];
y_out_real = iqimb_f1*real(temp(n_ssfilter_taps - 1 - iqimb_intdelay + (1:n_samples_out)));
y_out_imag = iqimb_f2*real(temp(n_ssfilter_taps - 1 - iqimb_intdelay + (1:n_samples_out)));
for ii = 1:n_ssfilter_taps
    y_out_imag = y_out_imag + iqimb_ssdelayfilt(n_ssfilter_taps - ii + 1)*imag(temp(ii - 1 + (1:n_samples_out)));
end
y_out_mat = complex(y_out_real, y_out_imag);

% VSPA implementation
temp = [zeros(n_ssfilter_taps-1, 1); y_lastddcstage_vsp];
y_out_real = r_half(r_smad(r_single(iqimb_f1), real(temp(n_ssfilter_taps - 1 - iqimb_intdelay + (1:n_samples_out))), 0));
y_out_imag = r_smad(r_single(iqimb_f2), real(temp(n_ssfilter_taps - 1 - iqimb_intdelay + (1:n_samples_out))), 0);
for ii = 1:n_ssfilter_taps
    y_out_imag = r_smad(r_single(iqimb_ssdelayfilt(n_ssfilter_taps - ii + 1)), imag(temp(ii - 1 + (1:n_samples_out))), y_out_imag);
end
y_out_imag = r_half(y_out_imag);
y_out_vsp = complex(y_out_real, y_out_imag);
