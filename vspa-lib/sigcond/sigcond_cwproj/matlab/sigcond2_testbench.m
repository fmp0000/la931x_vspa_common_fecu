...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function sigcond2_testbench(BW, N, n_batches, freq, fegain, dcoff, n_iqsstaps, testname)
...
if BW == 160
    ...
    x_input = [zeros(n_iqsstaps-1, 1); x_vsp];
end

if opmode == 1
    [temp, tempmat, opstruct] = r_customsigcond2(x_input, BW, filt_taps, sigcondcfg_struct);
else
    [temp, tempmat, ~] = r_customsigcond2(x_input, BW, filt_taps, sigcondcfg_struct);
end
...
fprintf('SQNR for sigcond2 = %.2f dB\n', sqnr);
...
if opmode == 1
    ...
    for jj = 1:n_batches
        drCW = dmemCreate(0, M);
        drCW = dmemWriteComplex(drCW, 0, opstruct.y_stage2((jj-1)*M + (1:M)), 'half_fixed');
...
drCW = dmemWriteReal(drCW, 115, iqimb_intdelay*2+1, 'uint');
