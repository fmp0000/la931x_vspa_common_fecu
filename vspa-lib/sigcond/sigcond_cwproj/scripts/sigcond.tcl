...
# load filter taps
sim_dram_load "${tv_path}/input_filtertaps.hex" [evaluate #x customsigcond_FilterTaps]

# configure sig cond parameters
sim_dram_load "${tv_path}/${test_name}_input_sigcondstruct.hex" [evaluate #x sigcondcfg_struct]
...
for {set i 0} {$i < $n_batches} {incr i} {
	...
	if {$odd_flag == 0} {
		...
		if {$opmode == 0} {
			...
			sim_dram_save "${tv_path}/output_y_batch$index\_cas.hex" [evaluate #x customsigcond_ScratchMem] $output_size_hword
		}
	} else {
		# load input data
