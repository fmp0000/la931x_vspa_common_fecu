...
################################################################ 
proc csp_chf_rcpv_case {L} {
    global tv_path
	set str_case "L${L}_"
    set n_word_y [expr (${L}*32) *2]
    set n_hword_y [expr (${L}*32*4)]
    puts " === testing... ${str_case}."
    
    ##########################    
    sim_dram_write [evaluate #x &L] ${L}
    sim_dram_load "${tv_path}/${str_case}x_in.hex" [evaluate #x x] "m"
...
# Debug
debug csp_chf_rcpv_cas
#debug csp_chf_rcpv_rdb
...
for {set iL 1}  {$iL  <= $nL}  {incr iL} {    
    ...
    set listPass [csp_chf_rcpv_case $L]
