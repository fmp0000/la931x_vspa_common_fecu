...
set ReportPath    "../../doc/Dot_Prod_Report.csv"
...
for {set TC_ind $TC_NUM_START} {$TC_ind <= $TC_NUM_END} {incr TC_ind} {
    ...
    restore -b "${tcDirPathIn}/ctrl.bin" m:[evaluate #x DOT_PROD_TEST_CTRL] 16bit

    # Load input buffers
    restore -b "${tcDirPathIn}/input1.bin" m:[evaluate #x DOT_PROD_TEST_INP1_LINE_BUFF] 16bit
    restore -b "${tcDirPathIn}/input2.bin" m:[evaluate #x DOT_PROD_TEST_INP2_LINE_BUFF] 16bit
    ...
    # Store dot product value
    save -b m:[evaluate #x &dot_prod]#4 "${tcDirPathOut}/dot_prod.bin" -o 16bit
    
    # Set local error count
    set LocalErrorCount  0
    
    # Verify dot product
    set diffResult [diffFiles "${tcDirPathOut}/dot_prod.bin" "${tcDirPathRef}/dot_prod.bin"]
    if {$diffResult != $DIFF_FILE_MATCH} then {
        incr TotalErrorCount
        incr LocalErrorCount
        lappend Failures "Correlation power error for TC ${TC_num} ..."
    }
