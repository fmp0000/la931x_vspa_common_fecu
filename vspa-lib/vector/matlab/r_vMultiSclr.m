...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function [y_vsp, y_mat, dbg] = r_vMultiSclr(x, alpha, out_prec)
% DESCRIPTION: 
%               y = x * alpha, where x,y are Nx1 vector, alpha is a scalar.
%  
% INPUTS:
%   x:          nx1 in real or complex.     Precision depends on SP_HPswitch.
%   alpha:      a real or complex scalar.   Precision depends on SP_HPswitch. 
...
%% vspa output
tmp = r_smad(alpha*ones(size(x)), x, zeros(size(x)));
if(out_prec==0)
    y_vsp = r_single(tmp);
elseif(out_prec==1)
